...
void HandleCollision(tObject *posObj)
{
	...
	{
		/* Save next pointer BEFORE any processing. KillObject→RemoveObject
		 * calls free() which invalidates the memory on modern platforms.
		 * On classic Mac OS, freed memory stayed readable, so the original
		 * code accessed theObj after KillObject. We must not do that. */
		...
		if(theObj!=posObj&&theObj!=posObj->shooter&&posObj!=theObj->shooter)
		{
			...
			if(sqdist<kMaxCollDist*kMaxCollDist)
				if(theObj->type && posObj->type && TestCollision(posObj,theObj,sqdist))
				{
					...
					UInt16 theFlags  = theObj->type->flags;
					UInt16 theFlags2 = theObj->type->flags2;
					UInt16 posFlags2 = posObj->type->flags2;
					int theKilled = 0;
					int posKilled = 0;

					if(theFlags & kObjectBounce)
					{
						if(posFlags2 & kObjectMissile){
							posObj->jumpHeight=0;
							posObj->jumpVelo=0;
							KillObject(theObj);
							theKilled = 1;
					...
					if(!posKilled && VEC2D_Value(posObj->velo)>kMinJumpVelo)
					{
						int jumpScore=0;
						if(!theKilled && theFlags2&kObjectRamp)
							{posObj->jumpVelo=VEC2D_Value(posObj->velo);jumpScore=theObj->type->score;}
						if(!theKilled && theFlags2&kObjectBump)
					...
					if(!posKilled && theFlags&kObjectKillsCars)
						{ KillObject(posObj); posKilled = 1; }
					if(!theKilled && theFlags&kObjectKilledByCars)
						{ KillObject(theObj); theKilled = 1; }
					if(!posKilled && theFlags2&kObjectOil)
						posObj->rotVelo=(fabs(posObj->rotVelo)>0.2?1:0)*(posObj->rotVelo>0?1:-1)*PI*0.05*VEC2D_Value(posObj->velo);
					if(!theKilled && posObj==gPlayerObj)
					{
						if(theFlags&kObjectBonusFlag)
							{ BonusObject(theObj); theKilled = 1; }
					}
					else if(!theKilled && !posKilled && theObj==gSpikeObj)
						if(posFlags2&kObjectDamageble)
						{
					...
					/* If posObj was killed, stop iterating - it's freed */
					if(posKilled)
						return;
				}
		}
