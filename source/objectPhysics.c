...
void ObjectPhysics(tObject *theObj)
{
	...
	if(!theObj->jumpHeight||theObj->type->flags&kObjectHeliFlag)
	{
		...
		/* Cache all type flags BEFORE any KillObject calls to prevent
		 * use-after-free: KillObject may call RemoveObject→free(theObj),
		 * making all subsequent theObj->type accesses invalid. */
		{
		UInt16 phFlags  = theObj->type->flags;
		UInt16 phFlags2 = theObj->type->flags2;
		int killed = 0;
		if(phFlags2&kObjectMissile){
		...
		else if(fabs(theObj->pos.y-gCameraObj->pos.y)<kVisDist)
		{
			...
			if(!killed && phFlags2&kObjectDamageble){
		...
		else if(!killed && phFlags2&kObjectDieWhenOutOfScreen)
			RemoveObject(theObj);
		}
