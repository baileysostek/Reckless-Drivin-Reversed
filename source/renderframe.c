...
int DrawComplCount()
{
	...
	{
		if(gPrefs.lapRecords[gLevelID]==0 || gPrefs.lapRecords[gLevelID]>gGameTime)
		{
	...
	if(gFinishDelay>=kStartBonusDelay+countTime+kCloseDelay)
	{
		...
		/* Check framebuffer guard zones before level transition */
		extern void CheckGuardZonesExtern(const char *ctx);
		CheckGuardZonesExtern("pre-DisposeLevel");
		DisposeLevel();
		gLevelID++;
		return LoadLevel();
	}
