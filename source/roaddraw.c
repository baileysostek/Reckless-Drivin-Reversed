...
static inline Ptr DrawBorder(Ptr drawPos,int xDrawStart,int x1,int x2,int y,Ptr data)
{
	int v=(-y&0x007f)<<4;
	int u=0;
...
{
	int leftBordEnd,rightBordEnd;
	int clampedX1 = x1 < -1024 ? -1024 : x1;
	int clampedX2 = x2 > gXSize + 1024 ? gXSize + 1024 : x2;
	leftBordEnd=clampedX1+16/zoom;
	rightBordEnd=clampedX2-16/zoom;
	if(leftBordEnd>rightBordEnd)
	{
		leftBordEnd=clampedX1+((clampedX2-clampedX1)>>1);
		rightBordEnd=leftBordEnd;
	}
...
void DrawRoadZoomed(float xDrawStart,float yDrawStart,float zoom)
{
	...
	for(screenY=0;screenY<gYSize;screenY+=(gPrefs.lineSkip?2:1))
	{
		...
		tRoadSeg roadData;
		int ceilIdx=(int)(ceilRoadLine);
		int floorIdx=(int)(floorRoadLine);
		tRoad ceilRoad,floorRoad;
		int ceilSplit,floorSplit;
		if(ceilIdx>=*gRoadLenght) ceilIdx=*gRoadLenght-1;
		if(floorIdx>=*gRoadLenght) floorIdx=*gRoadLenght-1;
		ceilRoad=(gRoadData+ceilIdx);
		floorRoad=(gRoadData+floorIdx);
		ceilSplit=(*ceilRoad)[1]!=(*ceilRoad)[2];
		floorSplit=(*floorRoad)[1]!=(*floorRoad)[2];
...
{
	int leftBordEnd,rightBordEnd;
	/* Clamp sentinel values to screen-relevant range before float arithmetic
	 * to avoid integer overflow (0x7fffffff promoted to float exceeds INT_MAX,
	 * causing undefined behavior when cast back to int). */
	int clampedX1 = x1 < -1024 ? -1024 : x1;
	int clampedX2 = x2 > gXSize + 1024 ? gXSize + 1024 : x2;
	leftBordEnd=clampedX1+16/zoom;
	rightBordEnd=clampedX2-16/zoom;
