...
void LoadSprites()
{
	...
	/* Byte-swap sprite headers (xSize, ySize are UInt16, rest are UInt8) */
	for(i=0;i<kNumSprites;i++)
	{
		if(gSprites[i])
		{
			tSpriteHeader16 *hdr = (tSpriteHeader16*)*gSprites[i];
			hdr->xSize = SwapU16(hdr->xSize);
			hdr->ySize = SwapU16(hdr->ySize);
		}
	}
}
...
void DrawLifeBar16(int cy,int cx,int shift)
{
	...
	int startU=shift;
	int endV=sprite->ySize;
	UInt16 *dst=(UInt16*)(gBaseAddr+y*gRowBytes+2*x);
	for(;v<endV;v++)
	{
		int u=startU;
		for(;u<endU;u++)
